...
class ServerStatus {
    ...
    static async send_keep_alive(): Promise<boolean> {
        try {
            const { stdout, stderr } = await promised_exec(
                `sshpass -p ${process.env.SUNFIRE_TUNNEL_PASS} ssh ${process.env.SUNFIRE_TUNNEL_USER}@${process.env.SUNFIRE_ADDRESS} 'echo ok'`,
            );
            if (stderr || !stdout) {
                return false;
            }
